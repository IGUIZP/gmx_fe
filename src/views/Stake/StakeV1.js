...
function getBalanceAndSupplyData(balances) {
  ...
  const keys = ["usdg", "gmt", "xgmt", "gmtUsdg", "xgmtUsdg", "gmtUsdgFarm", "xgmtUsdgFarm", "autoUsdg", "autoUsdgFarm"]
  const balanceData = {}
  const supplyData = {}
  const propsLength = 2
...
function getStakingData(stakingInfo) {
  ...
  const keys = ["usdg", "xgmt", "gmtUsdgFarmXgmt",
    "gmtUsdgFarmNative", "xgmtUsdgFarmXgmt", "xgmtUsdgFarmNative",
    "autoUsdgFarmXgmt", "autoUsdgFarmNative"
  ]
  const data = {}
  const propsLength = 2
...
function getProcessedData(balanceData, supplyData, stakingData, totalStakedData, pairData, xgmtSupply) {
  ...
  const xgmtPrice = pairData.xgmtUsdg.balance0.eq(0) ? bigNumberify(0) : pairData.xgmtUsdg.balance1.mul(PRECISION).div(pairData.xgmtUsdg.balance0)
  const gmtUsdgPrice = supplyData.gmtUsdg.eq(0) ? bigNumberify(0) : pairData.gmtUsdg.balance1.mul(PRECISION).mul(2).div(supplyData.gmtUsdg)
  const xgmtUsdgPrice = supplyData.xgmtUsdg.eq(0) ? bigNumberify(0) : pairData.xgmtUsdg.balance1.mul(PRECISION).mul(2).div(supplyData.xgmtUsdg)
  const bnbPrice = pairData.bnbBusd.balance1.mul(PRECISION).div(pairData.bnbBusd.balance0)
  const autoUsdgPrice = supplyData.autoUsdg.eq(0) ? bigNumberify(0) : pairData.autoUsdg.balance1.mul(PRECISION).mul(2).div(supplyData.autoUsdg)

  const usdgAnnualRewardsUsd = stakingData.usdg.tokensPerInterval.mul(bnbPrice).mul(HOURS_PER_YEAR).div(expandDecimals(1, 18))
  const xgmtAnnualRewardsUsd = stakingData.xgmt.tokensPerInterval.mul(bnbPrice).mul(HOURS_PER_YEAR).div(expandDecimals(1, 18))

  const gmtUsdgAnnualRewardsXmgtUsd = stakingData.gmtUsdgFarmXgmt.tokensPerInterval.mul(xgmtPrice).mul(HOURS_PER_YEAR).div(expandDecimals(1, 18))
  const gmtUsdgAnnualRewardsNativeUsd = stakingData.gmtUsdgFarmNative.tokensPerInterval.mul(bnbPrice).mul(HOURS_PER_YEAR).div(expandDecimals(1, 18))
  const gmtUsdgTotalAnnualRewardsUsd = gmtUsdgAnnualRewardsXmgtUsd.add(gmtUsdgAnnualRewardsNativeUsd)

  const xgmtUsdgAnnualRewardsXmgtUsd = stakingData.xgmtUsdgFarmXgmt.tokensPerInterval.mul(xgmtPrice).mul(HOURS_PER_YEAR).div(expandDecimals(1, 18))
  const xgmtUsdgAnnualRewardsNativeUsd = stakingData.xgmtUsdgFarmNative.tokensPerInterval.mul(bnbPrice).mul(HOURS_PER_YEAR).div(expandDecimals(1, 18))
  const xgmtUsdgTotalAnnualRewardsUsd = xgmtUsdgAnnualRewardsXmgtUsd.add(xgmtUsdgAnnualRewardsNativeUsd)

  const autoUsdgAnnualRewardsXgmtUsd = stakingData.autoUsdgFarmXgmt.tokensPerInterval.mul(xgmtPrice).mul(HOURS_PER_YEAR).div(expandDecimals(1, 18))
  const autoUsdgAnnualRewardsNativeUsd = stakingData.autoUsdgFarmNative.tokensPerInterval.mul(bnbPrice).mul(HOURS_PER_YEAR).div(expandDecimals(1, 18))
  ...
  data.usdgApr = data.usdgTotalStaked.eq(0) ? undefined : usdgAnnualRewardsUsd.mul(BASIS_POINTS_DIVISOR).div(totalStakedData.usdg).mul(expandDecimals(1, 18)).div(PRECISION)
  ...
  data.xgmtApr = data.xgmtSupplyUsd.eq(0) ? bigNumberify(0) : xgmtAnnualRewardsUsd.mul(BASIS_POINTS_DIVISOR).div(data.xgmtTotalStakedUsd)
  ...
  data.gmtUsdgApr = data.gmtUsdgSupplyUsd.eq(0) ? bigNumberify(0) : data.gmtUsdgFarmSupplyUsd.eq(0) ? undefined : gmtUsdgTotalAnnualRewardsUsd.mul(BASIS_POINTS_DIVISOR).div(data.gmtUsdgSupplyUsd)
  ...
  data.xgmtUsdgApr = data.xgmtUsdgFarmSupplyUsd.eq(0) ? undefined : xgmtUsdgTotalAnnualRewardsUsd.mul(BASIS_POINTS_DIVISOR).div(data.xgmtUsdgFarmSupplyUsd)
  ...
  data.autoUsdgApr = data.autoUsdgFarmSupplyUsd.eq(0) ? bigNumberify(0) : autoUsdgTotalAnnualRewardsUsd.mul(BASIS_POINTS_DIVISOR).div(data.autoUsdgFarmSupplyUsd)
...
function StakeModal(props) {
  const { isVisible, setIsVisible, title, maxAmount, value, setValue,
    active, account, library, stakingTokenSymbol, stakingTokenAddress, farmAddress, chainId } = props
...
function UnstakeModal(props) {
  const { isVisible, setIsVisible, title, maxAmount, value, setValue, library, stakingTokenSymbol, farmAddress } = props
...
export default function StakeV1() {
  ...
  const { data: xgmtSupply, mutate: updateXgmtSupply } = useSWR([active, chainId, readerAddress, "getTokenSupply", xgmtAddress], {
    fetcher: fetcher(library, Reader, [XGMT_EXCLUDED_ACCOUNTS]),
  })

  const { data: balances, mutate: updateBalances } = useSWR(["Stake:balances", chainId, readerAddress, "getTokenBalancesWithSupplies", account || AddressZero], {
    fetcher: fetcher(library, Reader, [tokens]),
  })

  const { data: stakingInfo, mutate: updateStakingInfo } = useSWR([active, chainId, readerAddress, "getStakingInfo", account || AddressZero], {
    fetcher: fetcher(library, Reader, [yieldTrackers]),
  })

  const { data: totalStakedInfo, mutate: updateTotalStakedInfo } = useSWR([active, chainId, readerAddress, "getTotalStaked"], {
    fetcher: fetcher(library, Reader, [yieldTokens]),
  })

  const { data: pairInfo, mutate: updatePairInfo } = useSWR([active, chainId, readerAddress, "getPairInfo", ammFactoryAddressV2], {
  ...
  }, [active, library, updateXgmtSupply, updateBalances,
      updateStakingInfo, updateTotalStakedInfo, updatePairInfo])
  ...
  return(
    <div className="Stake Page page-layout">
      ...
      <div className="Stake-note">
        ...
        <a href="https://gambit.gitbook.io/gambit/staking" target="_blank" rel="noopener noreferrer">staking details</a>&nbsp;
        ...
        <div className="App-card primary">
          ...
          <div className="Stake-card-bottom App-card-content">
            ...
            <div className="Stake-info App-card-row">
              ...
              <div>
                {formatKeyAmount(processedData, "usdgBalance", 18, 2, true)} (${formatKeyAmount(processedData, "usdgBalance", 18, 2, true)})
              </div>
            </div>
            <div className="Stake-info App-card-row">
            ...
            <div className="Stake-info App-card-row">
              ...
              <div>
                {formatKeyAmount(processedData, "usdgTotalStaked", 18, 2, true)} (${formatKeyAmount(processedData, "usdgTotalStakedUsd", 30, 2, true)})
              </div>
            </div>
            <div className="Stake-info App-card-row">
              <div className="label">Total Supply</div>
              <div>
                {formatKeyAmount(processedData, "usdgSupply", 18, 2, true)} (${formatKeyAmount(processedData, "usdgSupplyUsd", 30, 2, true)})
              </div>
            </div>
            <div className="App-card-options">
              <Link className="App-button-option App-card-option" to="/trade" target="_blank" rel="noopener noreferrer">Get USDG</Link>
              {active && <button className="App-button-option App-card-option" onClick={ () => claim(usdgAddress, processedData.usdgRewards) }>Claim</button>}
        ...
        <div className="App-card">
          ...
          <div className="Stake-card-bottom App-card-content">
            ...
            <div className="Stake-info App-card-row">
              <div className="label">Staked</div>
              <div>
                {formatKeyAmount(processedData, "xgmtBalance", 18, 2, true)} (${formatKeyAmount(processedData, "xgmtBalanceUsd", USD_DECIMALS, 2, true)})
              </div>
            </div>
            <div className="Stake-info App-card-row">
              <div className="label">Wallet</div>
            ...
            <div className="Stake-info App-card-row">
              <div className="label">Total Staked</div>
              <div>
                {formatKeyAmount(processedData, "xgmtTotalStaked", 18, 2, true)} (${formatKeyAmount(processedData, "xgmtTotalStakedUsd", USD_DECIMALS, 2, true)})
              </div>
            </div>
            <div className="Stake-info App-card-row">
              <div className="label">Total Supply</div>
              <div>
                {formatKeyAmount(processedData, "xgmtSupply", 18, 2, true)} (${formatKeyAmount(processedData, "xgmtSupplyUsd", USD_DECIMALS, 2, true)})
              </div>
            </div>
            <div className="App-card-options">
              <a className="App-button-option App-card-option" href={buyXgmtUrl} target="_blank" rel="noopener noreferrer">Get xGMT</a>
              {active && <button className="App-button-option App-card-option" onClick={ () => claim(xgmtAddress, processedData.xgmtRewards) }>Claim</button>}
              {!active && <button className="App-button-option App-card-option" onClick={connectWallet}>Connect Wallet</button>}
            </div>
          </div>
        </div>
        <div className="App-card">
          ...
          <div className="Stake-card-bottom App-card-content">
            ...
            <div className="Stake-info App-card-row">
              <div className="label">Staked</div>
              <div>
                {formatKeyAmount(processedData, "gmtUsdgStaked", 18, 4, true)} (${formatKeyAmount(processedData, "gmtUsdgStakedUsd", 30, 2, true)})
              </div>
            </div>
            <div className="Stake-info App-card-row">
              <div className="label">Wallet</div>
              <div>
                {formatKeyAmount(processedData, "gmtUsdgBalance", 18, 2, true)} (${formatKeyAmount(processedData, "gmtUsdgBalanceUsd", USD_DECIMALS, 2, true)})
              </div>
            </div>
            <div className="App-card-row">
              <div className="label">Rewards</div>
              <div>
                {hasFeeDistribution && processedData.gmtUsdgNativeRewards && processedData.gmtUsdgNativeRewards.gt(0) && `${formatKeyAmount(processedData, "gmtUsdgNativeRewards", 18, 8, true)} WBNB, `}
            ...
            <div className="Stake-info App-card-row">
              <div className="label">Total Staked</div>
              <div>
                {formatKeyAmount(processedData, "gmtUsdgTotalStaked", 18, 4, true)} (${formatKeyAmount(processedData, "gmtUsdgTotalStakedUsd", 30, 2, true)})
              </div>
            </div>
            <div className="App-card-options">
              <a className="App-button-option App-card-option" href={buyGmtUrl} target="_blank" rel="noopener noreferrer">Get GMT</a>
              <a className="App-button-option App-card-option" href={addGmtUsdgLpUrl} target="_blank" rel="noopener noreferrer">Create</a>
              {active && <button className="App-button-option App-card-option" onClick={() => showUnstakeGmtUsdgModal()}>Unstake</button>}
              {active && <button className="App-button-option App-card-option" onClick={ () => claim(gmtUsdgFarmAddress, processedData.gmtUsdgTotalRewards) }>Claim</button>}
              {!active && <button className="App-button-option App-card-option" onClick={connectWallet}>Connect Wallet</button>}
            </div>
          </div>
        </div>
        <div className="App-card">
          ...
          <div className="Stake-card-bottom App-card-content">
            ...
            <div className="Stake-info App-card-row">
              <div className="label">Staked</div>
              <div>
                {formatKeyAmount(processedData, "xgmtUsdgStaked", 18, 4, true)} (${formatKeyAmount(processedData, "xgmtUsdgStakedUsd", 30, 2, true)})
              </div>
            </div>
            <div className="Stake-info App-card-row">
              <div className="label">Wallet</div>
              <div>
                {formatKeyAmount(processedData, "xgmtUsdgBalance", 18, 2, true)} (${formatKeyAmount(processedData, "xgmtUsdgBalanceUsd", USD_DECIMALS, 2, true)})
              </div>
            </div>
            <div className="App-card-row">
              <div className="label">Rewards</div>
              <div>
                {hasFeeDistribution && processedData.xgmtUsdgNativeRewards && processedData.xgmtUsdgNativeRewards.gt(0) && `${formatKeyAmount(processedData, "xgmtUsdgNativeRewards", 18, 8, true)} WBNB, `}
            ...
            <div className="Stake-info App-card-row">
              <div className="label">Total Staked</div>
              <div>
                {formatKeyAmount(processedData, "xgmtUsdgTotalStaked", 18, 4, true)} (${formatKeyAmount(processedData, "xgmtUsdgTotalStakedUsd", 30, 2, true)})
              </div>
            </div>
            <div className="App-card-options">
              <a className="App-button-option App-card-option" href={buyXgmtUrl} target="_blank" rel="noopener noreferrer">Get xGMT</a>
              <a className="App-button-option App-card-option" href={addXgmtUsdgLpUrl} target="_blank" rel="noopener noreferrer">Create</a>
              {active && <button className="App-button-option App-card-option" onClick={() => showUnstakeXgmtUsdgModal()}>Unstake</button>}
              {active && <button className="App-button-option App-card-option" onClick={ () => claim(xgmtUsdgFarmAddress, processedData.xgmtUsdgTotalRewards) }>Claim</button>}
              {!active && <button className="App-button-option App-card-option" onClick={connectWallet}>Connect Wallet</button>}
            </div>
          </div>
        </div>
        <div className="App-card">
          ...
          <div className="Stake-card-bottom App-card-content">
            ...
            <div className="Stake-info App-card-row">
              <div className="label">Staked</div>
              <div>
                {formatKeyAmount(processedData, "autoUsdgStaked", 18, 4, true)} (${formatKeyAmount(processedData, "autoUsdgStakedUsd", 30, 2, true)})
              </div>
            </div>
            <div className="Stake-info App-card-row">
              <div className="label">Wallet</div>
              <div>
                {formatKeyAmount(processedData, "autoUsdgBalance", 18, 2, true)} (${formatKeyAmount(processedData, "autoUsdgBalanceUsd", USD_DECIMALS, 2, true)})
              </div>
            </div>
            <div className="App-card-row">
              <div className="label">Rewards</div>
              <div>
                {formatKeyAmount(processedData, "autoUsdgXgmtRewards", 18, 4, true)} xGMT
              </div>
            </div>
            <div className="Stake-info App-card-row">
              <div className="label">Total Staked</div>
              <div>
                {formatKeyAmount(processedData, "autoUsdgTotalStaked", 18, 4, true)} (${formatKeyAmount(processedData, "autoUsdgTotalStakedUsd", 30, 2, true)})
              </div>
            </div>
            <div className="App-card-options">
              <a className="App-button-option App-card-option" href={buyAutoUrl} target="_blank" rel="noopener noreferrer">Get AUTO</a>
              <a className="App-button-option App-card-option" href={addAutoUsdgLpUrl} target="_blank" rel="noopener noreferrer">Create</a>
              {active && <button className="App-button-option App-card-option" onClick={() => showStakeAutoUsdgModal()}>Stake</button>}
              {active && <button className="App-button-option App-card-option" onClick={() => showUnstakeAutoUsdgModal()}>Unstake</button>}
              {active && <button className="App-button-option App-card-option" onClick={ () => claim(autoUsdgFarmAddress, processedData.autoUsdgTotalRewards) }>Claim</button>}
              {!active && <button className="App-button-option App-card-option" onClick={connectWallet}>Connect Wallet</button>}
            </div>
          </div>
        </div>
